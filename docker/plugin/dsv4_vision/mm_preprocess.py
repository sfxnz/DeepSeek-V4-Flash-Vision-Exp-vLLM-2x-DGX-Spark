...
class DeepseekV4VLMultiModalProcessor(
    ...
    def _apply_hf_processor_main(
        ...
        *,
        ...
        # This B12X processor.py hardcodes enable_hf_prompt_update=True
        # in `_apply_hf_processor`. Force the split path so PromptReplacement
        # expands `<｜deepseek_image｜>`.
    ...
    def _apply_hf_processor_text_only(
        ...
        # Parent text-only still calls `_apply_hf_processor_text_mm` and
        # pops `input_ids` from the stand-in BatchFeature.
    ...
    def _apply_hf_processor_mm_only(
        ...
        # Parent mm-only assumes HuggingFace ProcessorMixin
        # (`_merge_kwargs` plus `image_processor(images=...)`). This
        # stand-in is not a mixin; call it directly.
