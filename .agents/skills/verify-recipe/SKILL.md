---
name: verify-recipe
description: Verify the DeepSeek-V4-Flash-Vision-Exp vLLM recipe (this repo) with the vendored forge kit. Never docker rm glm53-flash-nvfp4. Lease → doctor → probes → bench → evidence under evidence/<unit>-<UTC>/. Use when proving a recipe boots, answers, and holds its published decode numbers. Shared GPU serve — never start a second instance, never stop one this run did not start.
---

# Verify DeepSeek-V4-Flash-Vision-Exp
...
python3 kit/bench_compare.py evidence/rebench-20260902T202935Z/bench.json "$EV/bench.json"   # ±5% on decode and aggregate
```
